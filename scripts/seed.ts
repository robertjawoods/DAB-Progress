...
const homeworkData: Prisma.HomeworkCreateArgs['data'][] = [
...
const lessonData: Prisma.LessonCreateArgs['data'][] = [
...
const challengeData: Prisma.ChallengeCreateArgs['data'][] = [
...
export default async () => {
  try {
    ...
    Promise.all(
      ...
      //
      [
        homeworkData.map(async (data) => {
          const record = await db.homework.create({ data })
          console.log(record)
        }),
        lessonData.map(async (data) => {
          const record = await db.lesson.create({ data })
          console.log(record)
        }),
        challengeData.map(async (data) => {
          const record = await db.challenge.create({ data })
          console.log(record)
        }),
      ]
    )
